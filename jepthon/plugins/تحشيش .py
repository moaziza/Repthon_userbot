...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 1183747742:
        return await edit_or_reply(mention, f"**بس يكلب ده المبرمج مصطفي العزايزي**")
    muh = user.first_name.replace("\u2060", "") if user.first_name else user.username
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 1183747742:
        return await edit_or_reply(mention, f"**العزايزي ميخون يبقى وفي للكل**")
    muh = user.first_name.replace("\u2060", "") if user.first_name else user.username
    rzona = random.choice(rr7)
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 1183747742:
        return await edit_or_reply(mention, f"**العزايزي حلو ميحتاج تقيمه 🤤**")
    muh = user.first_name.replace("\u2060", "") if user.first_name else user.username
    rzona = random.choice(rr7)
