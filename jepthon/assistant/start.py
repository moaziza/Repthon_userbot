...
async def start(event):
    ...
    if event.sender_id == bot.uid:
        ...
        await tgbot.send_message(
            ...
            link_preview=False,
            buttons=[
                [custom.Button.inline("تنـصيب العزايزي  🐍", data="deploy")],
                [Button.url("تحتاج مسـاعدة ❓", "https://t.me/php_7")],
            ],
        )
    ...
    if event.query.user_id is not bot.uid:
        await tgbot.send_message(
            ...
            buttons=[
                ...
                [Button.url("كروب المساعدة ❓", "https://t.me/BANDA1M")],
            ],
        )
...
async def settings(event):
    if event.sender_id == bot.uid:
        ...
        await event.answer("انت لا تستطيع استخدام البوت احصل على بوتك من @BANDA1M", alert=True)




@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"rozname"))) 
async def settings(event):  #    قـسـم  الزغرفـة العزايزي 
    if event.sender_id == bot.uid:
        await event.delete()
        await tgbot.send_message(event.chat_id,
...
async def users(event):
    await event.delete()
    rorza = "تستطيع اختراق اي شخص عبر كود تيرمكس في العزايزي يمكنك اختراق المستخدمين الذي تملك كود تيرمكس الخاص بهم \n\n ارسل  /rz للعرض الاوأمر"
    await tgbot.send_message(event.chat_id, rorza)
